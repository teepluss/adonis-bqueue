...
class MakeQueue extends Command {
  static get signature() {
    return `make:queue { name: Name of Job (Queue) }`;
  }
  ...
  async handle({
    ...
    try {
      ...
      const templatePath = join(__dirname, '../../templates/Queue.mustache');
      const templateContent = await this.readFile(templatePath, 'utf-8');
      const filePath = join('app/Queues', name) + '.js';
  ...
  getNamespace(filePath) {
    return `App/Queues/${basename(filePath).replace('.js', '')}`;
  }
  /**
  ...
  printInstructions(namespace) {
    const lines = ['Register job/queue as follows', '', `1. Open ${this.chalk.cyan('start/app.js')}`, `2. Add ${this.chalk.cyan(namespace)} to jobs array`];
